<template>
  ...
  <header
    ...
    class="top-0 z-30 h-[72px] backdrop-blur backdrop-filter firefox:bg-opacity-90">
    <div class="mx-auto max-w-8xl xl:px-8">
      ...
      <div class="flex items-center justify-between gap-8">
        <div class="nav-item" v-for="item in header" :key="item.name">
          ...
          <template v-else>
            <a class="transition-colors hover:text-gray-700 dark:hover:text-gray-400"
              :class="isActive(item.name) ? 'text-gray-900 dark:text-gray-400 111' : 'text-gray-400 dark:text-gray-500'"
        ...
        <a
          class="flex items-center justify-between text-gray-400 dark:text-gray-500 transition-colors hover:text-gray-700 dark:hover:text-gray-400"
        ...
        <a class="text-gray-400 dark:text-white transition-colors hover:text-gray-500" href="javascript:void(0);" @click="switchLang">
        ...
        <!-- <a href="javascript:void(0)" class="flex items-center justify-between text-gray-400 dark:text-gray-500 transition-colors hover:text-gray-700 dark:hover:text-gray-400 contact">
          {{ isZhLang ? '联系我们' :  'Contact Us'}}
          <img class="communication-img" src="https://m.hellobike.com/resource/helloyun/16682/EzhOb_WechatIMG440.jpeg?x-oss-process=image/quality,q_80"/>
        </a> -->
      </div>
    </div>
    </div>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const isHomePage = computed(() => {
        return function() {
          return !(route.path.includes("component") || route.path.includes("guide"))
        }
      });
...
<style lang="scss" scoped>
  .contact:hover .communication-img {
    visibility:visible
  }
  .communication-img {
    width: 250px;
